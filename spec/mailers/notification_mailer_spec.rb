...
RSpec.describe NotificationMailer, type: :mailer do
  ...
  describe "#scholarship_agreement_declined_fyi" do
    ...
    let!(:allocation) { create(:allocation, source: scholarship, allocatable: registration, amount: 5_000) }
    let(:notification) do
      # Fresh load so notification.noticeable resolves its allocation from the DB
      # (as the mailer job does), not a stale in-memory instance.
      create(:notification, kind: "scholarship_agreement_declined_fyi", noticeable: Scholarship.find(scholarship.id), custom_message: "Timing no longer works")
    end

    it "renders without raising" do
      expect { described_class.scholarship_agreement_declined_fyi(notification).deliver_now }.not_to raise_error
    end

    it "names the recipient in the subject" do
      expect(described_class.scholarship_agreement_declined_fyi(notification).subject).to include("Scholarship declined by #{person.full_name}")
    end

    it "includes the reason and the event in the body" do
      body = described_class.scholarship_agreement_declined_fyi(notification).body.encoded
      expect(body).to include("Timing no longer works")
      expect(body).to include("Facilitator Training")
    end
  end
